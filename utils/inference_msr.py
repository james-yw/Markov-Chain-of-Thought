...
class ReactSolver(object):
    ...
    def __init__(self, args, question: str):
        ...
        self.step_solutions = [] # with_original_format, for constructing new dataset

        self.step_solutions_for_gpt = [] # without_original_format, for gpt4 generation
    ...
    def process_step_result(
        ...
        if parser_result is None:
            ...
            # update step_solutions
            thought = re.findall(r'<p>(.*?)<\/p>', new_step.text, re.DOTALL)
            thought = [item.strip() for item in thought]
            thoughts = "\n".join(thought)
            self.step_solutions.append({
                "thought": thoughts,
                "action": "",
                "action_input": "",
            })

            # update step_solutions_for_gpt
            thought = new_step.text.split("Final Answer:")[0]
            thought = thought.split("Action:")[0]
            thought = thought.split("Thought:")[1].strip() if "Thought:" in thought else thought
            self.step_solutions_for_gpt.append({
                "thought": thought,
                "action": "",
                "action_input": "",
            })
            ...
            # get observation
            observation = action_execution(self.step_actions).strip()
            ...
            self.step_solutions.append({
                ...
                "action_input": new_step.action_input,
