...
from inference import Solver, STOP
...
def updater(solver, output):
    solver.update_question(output)
    return solver
...
def batch_main(args, llm, sampling_params, questions, max_depth):
    ...
    try:
        solvers = [Solver(args, question) for question in questions]

        for step in tqdm(range(max_depth), desc="Step Processing"):
            
            ### MCoT Stage1: Derivation
            prompts = []
            stop_words = []
            epoch_solvers = []
            next_solvers = []
            ...
            for solver in solvers:
                if solver.terminal_flag():
                    prompt_text = solver.get_question_request()
                    prompts_2.append(prompt_text)
                    stop_words.extend(STOP)
                    epoch_solvers.append(solver)
                else:
                    next_solvers.append(solver)
            ...
            with ProcessPool(max_workers=min(len(epoch_solvers), os.cpu_count())) as pool:
                future = pool.map(updater, epoch_solvers, outputs, timeout=TIMEOUT_SECONDS)
                iterator = future.result()
...
def main(args):
    ...
    if os.path.exists(save_file):
        print(colored(f"File {save_file} already exists, please backup it first.", "red"))
        save_file = os.path.join(args.save_dir, os.path.basename(args.question_file) + f".prediction.with_{args.checkpoint_dir.split('/')[-1]}." + time.strftime("%Y%m%d%H%M%S") + ".jsonl")
...
def parse_args():
    ...
    args.add_argument('-s', '--save_dir', type=str, required=True, help="folder to save prediction file.")
